...
def login():
    ...
    if auth.password == usuario.senha:
        token = jwt.encode({'id_autor': usuario.id_autor, 'exp': datetime.now(timezone.utc) + timedelta(minutes=5)},
...
def obter_autor_por_id(id_autor):
    ...
    if not autor:
        return jsonify(f'Autor não encontrafo')
    autor_atual={}
    autor_atual['id_autor'] = autor.id_autor
    autor_atual['nome'] = autor.nome
    autor_atual['email'] = autor.email
...
def alterar_autor(id_autor):
    ...
    autor = Autor.query.filter_by(id_autor=id_autor).first()
    if not autor:
        return jsonify({'Menssagem':'Este usuario não foi encontrado'})
    try:
